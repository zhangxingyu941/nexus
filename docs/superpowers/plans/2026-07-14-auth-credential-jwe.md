...
**Goal:** 将登录密码、注册密码、重置密码和邮箱验证码改为浏览器 JWE 加密、服务端解密，并通过 60 秒一次性 challenge 阻止密文重放。

**Architecture:** 浏览器先从 challenge API 获取服务端签名 challenge 和 RSA 公钥 JWK，再使用 `RSA-OAEP-256 + A256GCM` 生成 Compact JWE。服务端在现有限流之后统一解密、校验用途与邮箱、原子消费 challenge，再把临时明文交给现有 Argon2id 认证存储层；生产使用 Redis，开发无 Redis时使用内存消费记录。

**Tech Stack:** Next.js 15、React 18、TypeScript、jose 5.10、Node Crypto、Redis 7、Vitest、Testing Library、Playwright。
...
- Modify: `.gitignore`
- Create: `src/shared/authCredential.ts`
- Create: `src/server/authCredentialKey.ts`
- Create: `src/server/authCredentialKey.test.ts`
- Create: `scripts/generate-auth-credential-key.ts`

- [x] **Step 1: 添加 jose 5.10 和密钥生成脚本入口**

Run: `pnpm add jose@5.10.0`
...
Expected: `package.json` 与 `pnpm-lock.yaml` 只新增 `jose@5.10.0` 相关依赖。

- [x] **Step 2: 先写密钥加载失败测试**

覆盖缺少 `AUTH_CREDENTIAL_KEY_ID`、缺少文件路径、文件不存在、不是 PKCS#8 RSA 私钥，以及成功派生不含私钥参数 `d` 的公开 JWK。

Run: `pnpm test --run src/server/authCredentialKey.test.ts`

Expected: FAIL，因为 `loadAuthCredentialKey` 尚不存在。
...
## Task 2：Challenge 签名与防重放存储

**Files:**
- Create: `src/server/authCredentialReplayStore.ts`
- Create: `src/server/authCredentialReplayStore.test.ts`
- Create: `src/server/authCredentialService.ts`
- Create: `src/server/authCredentialService.test.ts`

- [x] **Step 1: 先写内存和 Redis 原子消费测试**
...
Run: `pnpm test --run src/server/authCredentialReplayStore.test.ts`

Expected: FAIL，因为 replay store 尚不存在。

- [x] **Step 2: 实现 replay store**
...
- [x] **Step 3: 先写 challenge 与 JWE 服务测试**
...
Run: `pnpm test --run src/server/authCredentialService.test.ts`

Expected: FAIL，因为服务尚未实现。

- [x] **Step 4: 实现 challenge 和解密服务**
...
Run: `pnpm test --run src/server/authCredentialReplayStore.test.ts src/server/authCredentialService.test.ts`

Expected: PASS，测试输出和异常不包含测试密码或验证码。

## Task 3：Challenge API 与错误契约

**Files:**
- Create: `src/app/api/auth/credential-challenge/handlers.ts`
- Create: `src/app/api/auth/credential-challenge/route.ts`
- Create: `src/app/api/auth/credential-challenge/route.test.ts`
- Create: `src/app/api/auth/authCredentialResponse.ts`
- Create: `src/app/api/auth/authCredentialResponse.test.ts`
- Modify: `src/server/authRequestSecurity.ts`

- [x] **Step 1: 写失败的 challenge 路由与错误映射测试**

覆盖合法 purpose 返回 `{ algorithm, challenge, expiresAt, key }` 与 `Cache-Control: no-store`；非法 purpose 返回 `400`；限流返回 `429`；密钥/Redis配置错误返回不泄漏内部异常的 `503`。

Run: `pnpm test --run src/app/api/auth/credential-challenge/route.test.ts src/app/api/auth/authCredentialResponse.test.ts`

Expected: FAIL。

- [x] **Step 2: 增加按 IP 的 challenge 限流规则**
...
- [x] **Step 4: 运行 challenge API 测试**

Run: `pnpm test --run src/app/api/auth/credential-challenge/route.test.ts src/app/api/auth/authCredentialResponse.test.ts src/server/authRequestSecurity.test.ts`

Expected: PASS。

## Task 4：四个认证接口严格切换到密文
...
- Modify: 对应四个 `route.ts`
- Modify: 对应四个 `route.test.ts`

- [x] **Step 1: 把路由测试改为注入 credential service**
...
- [x] **Step 2: 运行四个路由测试确认失败**

Run: `pnpm test --run src/app/api/auth/session/route.test.ts src/app/api/auth/register/route.test.ts src/app/api/auth/verify-email/route.test.ts src/app/api/auth/password/reset/route.test.ts`

Expected: FAIL，因为 handler 仍读取明文字段。
...
**Files:**
- Create: `src/app/authClient.ts`
- Create: `src/app/authClient.test.ts`
- Modify: `src/app/AuthScreen.tsx`

- [x] **Step 1: 写失败的浏览器 JWE 测试**
...
Run: `pnpm test --run src/app/authClient.test.ts`

Expected: FAIL。

- [x] **Step 2: 实现 authClient**
...
- Modify: `e2e/editor.spec.ts` 或现有认证 E2E 文件

- [ ] **Step 1: 更新 UI fetch 测试序列**
...
- Modify: `README.md`
- Modify: `docs/prd.md`（只在现有认证描述中追加 JWE，不改动用户已有路线图内容）
- Generate, ignored: `.secrets/auth-credential-private.pem`
- Modify, ignored: `.env`

- [ ] **Step 1: 更新配置示例和 Compose**
...
Run: `pnpm auth:keygen`

Expected: 创建已忽略的 PKCS#8 PEM 文件，终端不显示私钥。使用 `apply_patch` 只向本地 `.env` 增加 `kid` 和路径，不改动或输出现有数据库、SMTP 等 Secret。
...
Expected: Next.js 生产构建成功。不会在每次任务后重复构建。

- [ ] **Step 4: 数据库模式冒烟与浏览器验收**
