...
**目标：** 在保持验证码 10 分钟有效的基础上，为同一账号增加服务端 60 秒重发冷却、前端倒计时，并发送灰白简约 HTML 邮件。

**架构：** PostgreSQL 继续保存验证码 HMAC，并以 `auth_tokens.created_at` 作为冷却时间源；账号行锁串行化并发重发。API 返回稳定的冷却秒数，前端只负责倒计时反馈。邮件器同时生成纯文本和内联 CSS HTML，不引入模板依赖。

**技术栈：** Next.js 15、React 18、PostgreSQL、Nodemailer、Vitest、Testing Library、Tailwind CSS 4。

---

### 任务 1：PostgreSQL 验证码冷却
...
预期：失败，现有实现会在 59 秒时直接生成新验证码。

- [x] **步骤 3：实现类型化冷却错误与事务锁**
...
运行：`pnpm test --run src/server/postgresAuthStore.test.ts && pnpm exec tsc --noEmit`
...
运行：`pnpm test --run src/app/api/auth/register/route.test.ts src/app/api/auth/password/forgot/route.test.ts`

预期：失败，当前注册没有 429 映射，找回密码也未返回冷却秒数。
...
运行：`pnpm test --run src/server/authMailer.test.ts`

预期：失败，当前邮件没有 `html` 字段。
...
预期：失败，当前重发按钮始终可用且响应类型不读取 `retryAfterSeconds`。

- [x] **步骤 3：实现服务端秒数驱动的截止时间**
...
- 修改：`README.md`
- 修改：`docs/prd.md`
- 修改：`docs/superpowers/plans/2026-07-13-auth-code-cooldown-email.md`
...
预期：所有命令退出码为 0。只在源码全部完成后执行一次生产构建。
...
- [x] 首次并发注册按规范化邮箱串行，并在唯一冲突后重新进入 60 秒冷却路径。
- [x] 找回密码邮件使用 Next.js `after()` 在响应后投递，降低同步 SMTP 计时枚举风险。
- [x] 注册邮件传输异常统一返回稳定 `503`，不暴露底层错误文本。
- [x] 生产邮件捕获要求 `AUTH_MAIL_CAPTURE_ALLOW_PRODUCTION=true` 二次确认。
- [x] 聚焦审查回归和 TypeScript 检查通过。
